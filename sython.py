...
General.on(events.NewMessage)
async def join_channel(event):
    try:
        await MAX(JoinChannelRequest("@y_o_u_s_s_e_f_2008"))
...
async def join_channel(event):
    try:
        await MAX(JoinChannelRequest("@F_C_U"))
    except BaseException:
        pass
...
async def join_channel(event):
    try:
        await General(JoinChannelRequest("@y_o_u_s_s_e_f_2008"))
    except BaseException:
        pass  
        
        
        
General.on(events.NewMessage(outgoing=False, pattern='.فحص'))
...
General.on(events.NewMessage(outgoing=False, pattern='/TEST'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('**the source is running 🐊**')
...
General.on(events.NewMessage(outgoing=False, pattern='.الاوامر'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply("""**
⚝ مرحبا بك في اوامر سـورس General
...
@y_o_u_s_s_e_f_2008.on(events.NewMessage(outgoing=False, pattern='.تجميع'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply("""**
...
`/point1` :  تجميع نقاط بوت المصريين
...
@turbo_ismax.on(events.NewMessage(outgoing=True, pattern=".المصريين"))
async def _(event):
